...
class Genotype:
    ...
    def loadSNPFile(self, filepath):
        ...
        if 'vcf' in file_extension:
            ...
            self.read23andme(data)

    # Method to load data from a buffer
    # Mostly used for web upload
    def loadSNPData(self, data):
        firstline = data.readline()
        print "firstline: " + firstline
        if firstline.find("VCF") >= 0:
            data.seek(0)
            self.readVCF(data)
        elif firstline.find("23andMe") >= 0:
            data.seek(0)
            self.read23andme(data)
        else:
            print "The file does not seem to be VCF of 23andme"
